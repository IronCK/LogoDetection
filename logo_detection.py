...
def main():
    ...
    if len(filename.split('/')) > 1:
        image, image_gry = get_image(filename)
    else:
    ...
    for template in template_files:
        ...
        if template.endswith('.png'):
            ...
            cv2.destroyAllWindows()
            """
            if not (image_gry.shape[0] <= template_to_match.shape[0] or image_gry.shape[1] <= template_to_match.shape[1]):
                ...
                if len(locations[1]) > 0:
                    found = True
                    print('Document belongs to the organization '+bank)
                    break
